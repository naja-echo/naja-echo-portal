...
describe('Crew Resources nav group', () => {
  ...
  it('leaves My Loot open to every authenticated member', () => {
    const myLoot = crewResourcesItems.find((item) => item.label === 'My Loot')
    expect(myLoot?.access).toBeUndefined()
  })

  it('gates Loot Distribution to the roles its endpoints require', () => {
    const distribution = crewResourcesItems.find((item) => item.label === 'Loot Distribution')
    expect(distribution?.access).toEqual([ROLES.CrewResourceOfficer, ROLES.Quartermaster])
  })
})
